...
class DeviceListScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListenableBuilder(listenable: deviceManager, builder: (context, _) {
        final devices = deviceManager.devices;
        return ListView.builder(padding: const EdgeInsets.all(16), itemCount: devices.length, itemBuilder: (context, index) {
            final device = devices[index];
            return Card(color: Colors.grey[900], margin: const EdgeInsets.only(bottom: 12), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
              child: ListTile(contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
                leading: Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: device.isOn ? device.color.withOpacity(0.2) : Colors.white10, shape: BoxShape.circle), child: Icon(device.icon, color: device.isOn ? device.color : Colors.grey)),
                title: Text(device.name, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                subtitle: Text(device.isOn ? "Đang bật" : "Đang tắt", style: TextStyle(color: Colors.white.withOpacity(0.5), fontSize: 12)),
                trailing: Switch(value: device.isOn, activeColor: Colors.white, activeTrackColor: device.color, inactiveTrackColor: Colors.grey[800], onChanged: (val) { deviceManager.toggleDevice(device.id, val); }),
                onTap: () { Navigator.push(context, MaterialPageRoute(builder: (context) => DeviceDetailScreen(deviceName: device.name, color: device.color))); },
              ),
            );
        });
      }),
...
class DeviceDetailScreen extends StatefulWidget {
  final String deviceName; final Color color;
