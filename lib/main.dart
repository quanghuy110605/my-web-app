...
class _SmartHome3DPageState extends State<SmartHome3DPage> {
  ...
  String _getIconName(SmartDevice device) {
    ...
    // Logic cho Gara: CodePoint của directions_car
    if (device.icon.codePoint == Icons.directions_car.codePoint) return device.isOn ? SmartIcons.car : SmartIcons.garageEmpty;
  ...
  Widget build(BuildContext context) {
    return ListenableBuilder(listenable: deviceManager, builder: (context, _) {
        final myHotspots = deviceManager.devices.map((device) {
          ...
          return SmartHotspot(
            id: device.id, position: device.position, normal: device.normal, color: device.color, iconName: iconName,
            panelBuilder: (context, toggle3D, changeColor) => SmartLightPanel(deviceId: device.id, onToggle3D: toggle3D, onChangeColor: changeColor)
          );
        }).toList();
...
// (Giữ nguyên class AddingModeOverlay và _SequencePainter)
...
class _AddingModeOverlayState extends State<AddingModeOverlay> with TickerProviderStateMixin {
  late AnimationController _rippleController; late AnimationController _borderController; bool _rippleFinished = false;
  @override void initState() { super.initState(); _rippleController = AnimationController(vsync: this, duration: const Duration(milliseconds: 1500)); _borderController = AnimationController(vsync: this, duration: const Duration(seconds: 2)); _rippleController.forward(); _rippleController.addStatusListener((status) { if (status == AnimationStatus.completed) { setState(() => _rippleFinished = true); _borderController.repeat(); } }); }
  @override void dispose() { _rippleController.dispose(); _borderController.dispose(); super.dispose(); }
  @override Widget build(BuildContext context) { return IgnorePointer(child: AnimatedBuilder(animation: Listenable.merge([_rippleController, _borderController]), builder: (context, child) { return CustomPaint(painter: _SequencePainter(rippleValue: _rippleController.value, borderValue: _borderController.value, isRippleFinished: _rippleFinished), size: Size.infinite); },)); }
}
class _SequencePainter extends CustomPainter {
  final double rippleValue; final double borderValue; final bool isRippleFinished;
  _SequencePainter({required this.rippleValue, required this.borderValue, required this.isRippleFinished});
  @override void paint(Canvas canvas, Size size) { final center = Offset(size.width / 2, size.height / 2); final rect = Rect.fromLTWH(0, 0, size.width, size.height); if (isRippleFinished) { final Paint borderPaint = Paint()..style = PaintingStyle.stroke..strokeWidth = 6.0..shader = SweepGradient(colors: const [Colors.red, Colors.orange, Colors.yellow, Colors.green, Colors.cyan, Colors.blue, Colors.purple, Colors.red], transform: GradientRotation(borderValue * 2 * math.pi)).createShader(rect); canvas.drawRect(rect, borderPaint); } if (!isRippleFinished) { double maxRadius = math.max(size.width, size.height); double currentRadius = maxRadius * rippleValue; double opacity = (1.0 - rippleValue).clamp(0.0, 1.0); final Paint ripplePaint = Paint()..style = PaintingStyle.stroke..strokeWidth = 25.0..maskFilter = const MaskFilter.blur(BlurStyle.normal, 10)..color = Colors.cyanAccent.withValues(alpha: opacity * 0.3); canvas.drawCircle(center, currentRadius, ripplePaint); final Paint corePaint = Paint()..style = PaintingStyle.stroke..strokeWidth = 5.0..maskFilter = const MaskFilter.blur(BlurStyle.normal, 2)..color = Colors.white.withValues(alpha: opacity * 0.2); canvas.drawCircle(center, currentRadius, corePaint); } }
  @override bool shouldRepaint(covariant _SequencePainter oldDelegate) => true;
}